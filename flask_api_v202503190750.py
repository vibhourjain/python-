...
def initiate_pbrun():
    ...
    if not service_account or not user_id:
    ...
    client_id = user_id[:7]  # Extract first 7 characters of user_id
    powerbroker_command = f"pbrun {unix_group} initbreakglass-{service_account} {client_id}"
    logging.info(f"Generated powerbroker command: {powerbroker_command}")
    
    return render_template_string(html_form, powerbrokerCommand=powerbroker_command, user_id=client_id)
...
def execute_pbrun():
    ...
    logging.info(f"Executing PowerBroker command for user ID: {user_id}")
