    ...
    if st.button("Generate Graphs"):
        ...
        try:
            ...
            for _, row in metadata_df.iterrows():
                ...
                # For the metric table (keep borders and formatting)
                rows, cols = metric_df.shape
                metric_shape = slide_daily.shapes.add_table(rows + 1, cols, Inches(5.5), Inches(0.5), Inches(4),
                                                            Inches(1.6))
                metric_table = metric_shape.table

                # Apply a table style with borders using the style_id
                # Medium Style 2 - '{284E427A-3D55-4303-BF80-6455036E1DE7}' (black borders, no blue)
                tbl = metric_shape._element.graphic.graphicData.tbl
                style_id = '{284E427A-3D55-4303-BF80-6455036E1DE7}'  # Medium Style 2 with black borders
                ...
                if not peak_df.empty:
                    ...
                    # Create a text box for peak information
                    peak_textbox = slide_daily.shapes.add_textbox(Inches(5.5), Inches(2.3), Inches(4), Inches(1))
