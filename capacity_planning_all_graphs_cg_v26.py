    ...
    if st.button("Generate Graphs"):
        ...
        try:
            ...
            for _, row in metadata_df.iterrows():
                ...
                # Set table style to Table Grid to ensure borders are visible
                metric_table.first_row = True
                metric_shape.table.style = 'Table Grid'
                
                # Add explicit border settings for each cell to ensure borders are visible
                for row in range(rows + 1):
                    for col in range(cols):
                        ...
                        # Set border properties for all sides of the cell
                        for border in ['top', 'right', 'bottom', 'left']:
                            setattr(cell.borders, border, True)
                            border_obj = getattr(cell.borders, border)
                            border_obj.color.rgb = RGBColor(0, 0, 0)
                ...
                if not peak_df.empty:
                    peak_table = slide_daily.shapes.add_table(len(peak_df), 1, Inches(5.5), Inches(2.3), Inches(4), Inches(1)).table
                    
                    # Set table style for peak table
                    for r in range(len(peak_df)):
                        ...
                        # Add borders to peak table cells
                        for border in ['top', 'right', 'bottom', 'left']:
                            setattr(cell.borders, border, True)
                            border_obj = getattr(cell.borders, border)
                            border_obj.color.rgb = RGBColor(0, 0, 0)
