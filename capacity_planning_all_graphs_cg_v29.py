    ...
    if st.button("Generate Graphs"):
        ...
        try:
            ...
            for _, row in metadata_df.iterrows():
                ...
                # Force white background and ensure visible borders
                ...
                if not peak_df.empty:
                    peak_table = slide_daily.shapes.add_table(len(peak_df), 1, Inches(5.5), Inches(2.3), Inches(4),
                                                              Inches(1)).table
                    for r in range(len(peak_df)):
                        cell = peak_table.cell(r, 0)
                        cell.text = str(peak_df["PK_ROW"].iloc[r])  # Set text first
                        p = cell.text_frame.paragraphs[0]
