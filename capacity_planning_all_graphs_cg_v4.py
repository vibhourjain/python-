...
if st.button("Generate Graphs"):
    ...
    for _, row in metadata_df.iterrows():
        ...
        # Format X-axis ticks
        x_dates = df['Period'].dt.to_period('D').drop_duplicates().sort_values()
        unique_dates = x_dates.dt.to_timestamp()

        if len(unique_dates) <= 35:
            ax.set_xticks(unique_dates)
        else:
            skip = len(unique_dates) // 35 + 1
            ax.set_xticks(unique_dates[::skip])

        ax.set_xticklabels([d.strftime('%b-%y') for d in ax.get_xticks()])
