    ...
    if st.button("Generate Graphs"):
        ...
        try:
            ...
            for _, row in metadata_df.iterrows():
                ...
                # Add report-level title as text box
                ...
                metric_table = slide_daily.shapes.add_table(rows + 1, cols, Inches(5.5), Inches(0.5), Inches(4), Inches(1.6)).table
                ...
                # Add borders to all cells of the metric table
                for row in range(rows + 1):
                    for col in range(cols):
                        cell = metric_table.cell(row, col)
                ...
                # Metric table borders, fill, and outline
                from pptx.oxml.ns import qn
                from pptx.oxml import parse_xml

                # XML to draw borders on all 4 sides (light blue)
                xml_border = '''
                                <a:tcBorders xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main">
                                  <a:lnL w="12700"><a:solidFill><a:srgbClr val="ADD8E6"/></a:solidFill></a:lnL>
                                  <a:lnR w="12700"><a:solidFill><a:srgbClr val="ADD8E6"/></a:solidFill></a:lnR>
                                  <a:lnT w="12700"><a:solidFill><a:srgbClr val="ADD8E6"/></a:solidFill></a:lnT>
                                  <a:lnB w="12700"><a:solidFill><a:srgbClr val="ADD8E6"/></a:solidFill></a:lnB>
                                </a:tcBorders>
                                '''
                ...
                if not peak_df.empty:
                    ...
                    for r in range(len(peak_df) + 1):
                        ...
                        p.font.color.rgb = RGBColor(0, 0, 0)
                        if r == 0:
                            cell.text = "Peak Summary"
                            p.font.bold = True
                        else:
                            cell.text = peak_df["PK_ROW"][r - 1]
                            p.font.size = Pt(9)

                        # Add border to peak table
                        tcPr = cell._tc.get_or_add_tcPr()
                        tcPr.append(parse_xml(xml_border))
                # if not peak_df.empty:
                ...
                #     header_p = peak_table.cell(0, 0).text_frame.paragraphs[0]
                #     header_p.font.bold = True
                #     header_p.font.color.rgb = RGBColor(0, 0, 0)
                #     peak_table.cell(0, 0).fill.solid()
                #     peak_table.cell(0, 0).fill.fore_color.rgb = RGBColor(255, 255, 255)
                #
